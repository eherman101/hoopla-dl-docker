...
#[CmdletBinding(DefaultParameterSetName='UserPassSingleTitle')]
param(
    [int64[]]
    $TitleId,
...
#    [Parameter(Mandatory,ParameterSetName='UserPassSingleTitle')]
    [string]
    $Username = "edward.ix.herman@gmail.com",
 
#    [Parameter(Mandatory,ParameterSetName='UserPassSingleTitle')]
    [string]
    $Password = "4T3XDnzagPQC~'Q",
...
Function Invoke-HooplaZipDownload
{
    ...
    #$url="$URL_HOOPLA_WS_BASE/patrons/downloads/$CircId/url"
    ...
    $res = Invoke-WebRequest -Uri "https://patron-api-gateway.hoopladigital.com/core/v2/patrons/downloads/$mediaKey/url" -Method Get -Headers $h -UserAgent $USER_AGENT -UseBasicParsing
    $headers_str = $h | ConvertTo-Json -Compress
    Write-Warning "https://patron-api-gateway.hoopladigital.com/core/v2/patrons/downloads/$mediaKey/url $headers_str"
    Write-Warning "$URL_HOOPLA_WS_BASE/patrons/downloads/$CircId/url"
...
Function Get-HooplaKey
{
    ...
    $h['Authorization'] = "Bearer $Token"
    $h['patron-id'] = $PatronId
    Write-Warning "https://patron-api-gateway.hoopladigital.com/license/downloads/book/key/$mediaKey"
#    Invoke-RestMethod -Uri "$URL_HOOPLA_LIC_BASE/downloads/$CircId/key" -Method Get -Headers $h -UserAgent $USER_AGENT
    Invoke-RestMethod -Uri "https://patron-api-gateway.hoopladigital.com/license/downloads/book/key/$mediaKey" -Method Get -Headers $h -UserAgent $USER_AGENT
}
...
Function Get-FileKeyKey
{
    ...
    $combined = "{0}:{1}:{0}" -f $mediaKey, $patronId
...
Function Decrypt-File
{
    ...
    $aesManaged.Padding = [Security.Cryptography.PaddingMode]::PKCS7
    $aesManaged.BlockSize = 128
    $aesManaged.KeySize = $FileKey.Length * 8
...
Function Remove-InvalidFileNameChars
{
    ...
    $invalidChars = [IO.Path]::GetInvalidFileNameChars() -join '' + "`n"
...
"@
        $xml | Out-File -LiteralPath $containerXmlPath -Encoding UTF8
 
        if ($PSVersionTable.PSVersion.Major -lt 6)
        {
            Remove-BomFromFile -Path $containerXmlPath -Destination "$containerXmlPath.tmp"
            Remove-Item -LiteralPath $containerXmlPath -Force -Confirm:$false
            Rename-Item -LiteralPath "$containerXmlPath.tmp" -NewName $containerXmlPath -Force
        }
    }
	...
	& "C:\Users\edwar\Nextcloud\hoopla downloader\epubzip.exe" $finalFileFullPath > $null 2>&1
    Pop-Location
...
$cmd = Get-Command -Name ".\epubzip.exe" -ErrorAction SilentlyContinue
...
#Write-Warning "to downolad $_ ($($toDownload))..."
#$toDownload = Get-HooplaTitleInfo -PatronId $PatronId -Token $token -TitleId 12066651
...
$toDownload | ForEach-Object -Process {
    ...
    if (!$UseExistingDownload)
    {
        $circFileName = (Join-Path -Path $tempFolder -ChildPath "$($circId).zip")
        Invoke-HooplaZipDownload -PatronId $patronId -Token $token -CircId $circId -OutFile $circFileName -mediaKey $mediaKey
    }
    ...
    $fileKey = Decrypt-FileKey -FileKeyEnc ([Convert]::FromBase64String($keyData)) -FileKeyKey $fileKeyKey
    #Write-Warning "to downolad ($($fileKey))..."
 
    if ($UseExistingDownload)
    {
        $encDir = $UseExistingDownload
    }
    else
    {
        ...
        Expand-Archive -LiteralPath $circFileName -DestinationPath $encDir -Force
    ...
    $zipFiles | ForEach-Object -Process {
	    ...
	    if ((($_.Extension -eq '.m4a') -and (Test-Mp4 -LiteralPath $_.FullName)) -or ($_.Extension -eq '.mp3') -or ($_.Extension -eq '.mp4'))
	    #if (($_.Extension -eq '.m4a') -or ($_.Extension -eq '.mp3')) -and (Test-Mp4 -LiteralPath $_.FullName))
	    #if ((($_.Extension -eq '.m4a') -or ($_.Extension -eq '.mp3')) -and (Test-Mp4 -LiteralPath $_.FullName))
            {
